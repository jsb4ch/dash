...
*Setup instructions for a Gitian build of Kash Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Kash
...
- [Building Kash Core](#building-kash-core)
...
Clone the git repositories for Kash Core and Gitian.
...
in the Kash Core repository under 'Fetch and create inputs' to install sources which require
...
To build Kash Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Kash Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/kash/.git/
...
For example:
```bash
URL=https://github.com/crowning-/kash.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit kash=${COMMIT} --url kash=${URL} ../kash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit kash=${COMMIT} --url kash=${URL} ../kash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit kash=${COMMIT} --url kash=${URL} ../kash/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the kash git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../kash/contrib/gitian-descriptors/*|sort|uniq )
...
./bin/gbuild --url kash=${BTCPATH},signature=${SIGPATH} ../kash/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/kash-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/kash-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/kash-osx-build.assert
```
