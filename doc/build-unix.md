UNIX BUILD NOTES
====================
Some notes on how to build Kash Core in Unix.

(for OpenBSD specific instructions, see [build-openbsd.md](build-openbsd.md))

Base build dependencies
-----------------------
Building the dependencies and Kash Core requires some essential build tools and libraries to be installed before.
...
Security
--------
To help make your Kash installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, Kash Core should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, Kash Core may be compiled in
...
Add `CC=egcc CXX=eg++ CPP=ecpp` to the dependencies build and the Kash Core build:
```bash
$ cd depends
