...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 KASH irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on kashcentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in kash.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("kash.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in kash.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Kash features */
    { "kash",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "kash",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "kash",               "gobject",                &gobject,                true,  {} },
    { "kash",               "voteraw",                &voteraw,                true,  {} },
