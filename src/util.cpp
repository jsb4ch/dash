...
const char * const BITCOIN_CONF_FILENAME = "kash.conf";
const char * const BITCOIN_PID_FILENAME = "kashd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "kash" is a composite category enabling all Kash-related debug output
                if(ptrCategory->count(std::string("kash"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KashCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KashCore
    // Mac: ~/Library/Application Support/KashCore
    // Unix: ~/.kashcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KashCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KashCore";
#else
    // Unix
    return pathRet / ".kashcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty kash.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override kash.conf
