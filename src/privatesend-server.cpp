...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all Kash related functionality
...
/*
    ...
    allow endless transaction that would bloat Kash and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all Kash specific functionality
