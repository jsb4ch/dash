...
#if defined(NDEBUG)
# error "Kash Core cannot be compiled without assertions."
#endif
                              ...
                              const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    BOOST_FOREACH(const CTxIn &txin, tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in Kash
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    int nMNPIPeriod = Params().GetConsensus().nMasternodePaymentsIncreasePeriod;
                                                                      ...
                                                                      // mainnet:
    if(nHeight > nMNPIBlock)                  ret += blockValue / 20; // 158000 - 55.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 1)) ret += blockValue / 20; // 175280 - 60.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 2)) ret += blockValue / 20; // 192560 - 65.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 3)) ret += blockValue / 40; // 209840 - 67.5%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 4)) ret += blockValue / 40; // 227120 - 70.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 5)) ret += blockValue / 40; // 244400 - 72.5%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 6)) ret += blockValue / 40; // 261680 - 75.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 7)) ret += blockValue / 40; // 278960 - 77.5%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 9)) ret += blockValue / 40; // 313520 - 80.0%
...
static int64_t nTimeKashSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    // KASH : CHECK TRANSACTIONS FOR INSTANTSEND
        ...
        for (const auto& tx : block.vtx) {
            ...
            for (const auto& txin : tx->vin) {
                ...
                if (instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx->GetHash()) {
                    ...
                    return state.DoS(10, error("ConnectBlock(KASH): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), hashLocked.ToString()),
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
                return state.DoS(10, error("ConnectBlock(KASH): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else {
        LogPrintf("ConnectBlock(KASH): spork is off, skipping transaction locking checks\n");
    }
    ...
    // KASH : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(KASH): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(KASH): couldn't find masternode or superblock payments"),
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(KASH): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeKashSpecific += nTime5 - nTime4;
    LogPrint("bench", "    - Kash specific: %.2fms [%.2fs]\n", 0.001 * (nTime5 - nTime4), nTimeKashSpecific * 0.000001);
