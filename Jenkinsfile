...
// which are part of the Kash repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="kash-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          if (hasCache) {
            sh "cd /kash-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /kash-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /kash-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /kash-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /kash-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /kash-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /kash-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
