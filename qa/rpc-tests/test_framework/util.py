...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "kash.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def wait_for_bitcoind_start(process, url, i):
    '''
    Wait for kashd to start. This means that RPC is accessible and fully initialized.
    Raise an exception if kashd exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('kashd exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir, num_nodes, cachedir, extra_args=None, redirect_stderr=False):
    ...
    if create_cache:
        ...
        # Create cache directories, run kashds:
        for i in range(MAX_NODES):
            datadir=initialize_datadir(cachedir, i)
            args = [ os.getenv("BITCOIND", "kashd"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0", "-mocktime="+str(GENESISTIME) ]
            if i > 0:
            ...
            logger.debug("initialize_chain: kashd started, waiting for RPC to come up")
    ...
    for i in range(num_nodes):
        ...
        initialize_datadir(test_dir, i) # Overwrite port/rpcport in kash.conf
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False, stderr=None):
    """
    Start a kashd and return RPC connection to it
    """
    datadir = os.path.join(dirname, "node"+str(i))
    if binary is None:
        binary = os.getenv("BITCOIND", "kashd")
...
def assert_start_raises_init_error(i, dirname, extra_args=None, expected_msg=None):
    with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
        try:
            ...
            assert 'kashd exited' in str(e) #node must have shutdown
            ...
            if expected_msg is None:
                assert_msg = "kashd should have exited with an error"
            else:
                assert_msg = "kashd should have exited with expected error " + expected_msg
            raise AssertionError(assert_msg)

def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False):
    """
    Start multiple kashds, return RPC connections to them
    """
...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s KASH too low! (Should be %s KASH)"%(str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s KASH too high! (Should be %s KASH)"%(str(fee), str(target_fee)))
